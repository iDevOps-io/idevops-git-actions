...
input_template_file_contents=open(os.environ["INPUT_INPUT_FILE"], "r").read()
output_template_file=open(os.environ["INPUT_OUTPUT_FILE"], "w")

for key in os.environ.keys():
    if str(key) in str(input_template_file_contents):
        print("Found {key} in file {input_file} contents replacing with value".format(key=key, input_file=os.environ["INPUT_INPUT_FILE"]))
...
os.remove(os.environ["INPUT_INPUT_FILE"])